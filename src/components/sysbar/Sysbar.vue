<template>
  <span class="user-info" v-if="userInfo">
  	<div v-show="showDiv" v-drag style="position: fixed;z-index: 3;width: 85%;left: 10%;right: 90%;">
  		<i class="el-icon-error" style="cursor: pointer;position: absolute;top: 7px;right: 4px;z-index: 100;" @click="handleClose()"></i>
  		<div ref="dlgRef" style="height: 550px;padding-top: 5px;line-height: 15px;"><p style="background: rgb(39, 41, 45);height: 15px;"></p>
  			<iframe ref="iframe1" :src="linkSrc" frameborder="0" framespacing="0" height="100%" width="100%"></iframe>
  		</div>
  	</div>
  	<div style="position: fixed;text-align: center;vertical-align:middle;z-index: 3;width: 85%;left: 10%;right: 90%;" v-show='fullscreenLoading'  >
			<span class="loading"><i class="el-icon-loading"></i>正在加载......</span>
		</div>
  	<i class="el-icon-news" v-show="false" style="cursor: pointer;" title="div" @click="handleDiv"></i>&nbsp;&nbsp;&nbsp;
    <i class="el-icon-bell" style="cursor: pointer;" title="即时通讯" @click="linkImMsg"><im></im></i>&nbsp;&nbsp;&nbsp;
    ...
    <el-dialog v-show="showDialog" class="dialogClass" size="small" :visible.sync="showIm" :show-close="false" :close-on-click-modal="true" @close="closeDialog" width="85%" top="7vh">
...
<style>
  .addPart{font-size: 14px;margin-right: 25px;vertical-align: top;cursor: pointer;}
  .addPart .iconfont{font-size: 16px;}
  ...
  .header .user-info .el-icon-bell{font-size: 14px;}
  .header .user-info .icon-yonghu{font-size: 14px;}
</style>
